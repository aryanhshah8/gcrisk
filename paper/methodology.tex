% ─────────────────────────────────────────────────────────────────────────────
% METHODS SECTION (condensed — full derivations in Supplementary Material)
...
a separate proton RBE self-consistency check. Full equations, parameter
...
approximation applied to per-species local interstellar spectra: protons
and helium come from Vos \& Potgieter \cite{vos2015}, and C/O/Si/Fe from
...
$16\gcm$ aluminum \citep{zeitlin2013} — a one-free-parameter calibration
...
\textbf{Transport.} Ionization energy loss uses the continuous-slowing-down
approximation (CSDA) with Barkas effective-charge scaling on NIST
...
free paths \citep{wilson1991} — which does not track secondary fragment
production; this is the model's primary physical limitation, most relevant
...
Point estimates use a fixed DDREF of $1.75$; the uncertainty ensemble
instead samples DDREF~$\sim$~Uniform$[1,2]$. Because REID enters an
exponential, its distribution is right-skewed, with \code{Q\_factor}
(\S\ref{sec:uq}) the dominant tail driver, clipped at the ICRP-60 physical
maximum of $30$ (Supplementary \S{}S1.6).
...
Nine parameters are varied simultaneously via Latin Hypercube Sampling
(LHS): five physics (solar modulation, LIS normalization, nuclear
...
(\code{narrow} argument to \code{gcr.uncertainty.lhs\_samples}; Supplementary
\S{}S1.9).
\label{sec:vovmethod}
...
(Table~\ref{tab:validation}), not independent experimental validation. This
layer is the foundation for a separate, planned proton-therapy crossover
study and is not a central contribution of the present paper.
