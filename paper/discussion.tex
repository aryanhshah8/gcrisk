...
uncertainties}, not as independently predicting the flight result. The
...
comparison for a few days, not a physics deficiency in the chronic-GCR model.
...
\subsection{REID Interpretation: Biology, Not Physics, Dominates}
...
order of magnitude," not as a precise prediction.
...
RBE self-consistency layer (\S\ref{sec:rbemethods}) reported only as a
implementation check (Table~\ref{tab:validation}); it is the foundation
for a planned, separate proton-therapy crossover study and is not a
central contribution here.
...
daily-scale claims and is the reason the ISS cross-check tests trend, not
absolute dose, and does not extend to near-Earth or trapped-radiation
...
(\S\ref{sec:rbemethods}) is the foundation for a planned, separate
proton-therapy crossover study, not yet a contribution of the present
paper.
...
this pipeline against HZETRN, OLTARIS, Geant4, and TOPAS: it is not a
replacement for these at high fidelity, but is, to my knowledge, the only
